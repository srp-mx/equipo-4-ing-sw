...
export const Sidebar = ({ incomplete }: { incomplete?: boolean }) => {
    return (
        <div
            className={`flex justify-around md:flex-col ${incomplete ? "md:row-span-4" : "md:col-span-2 md:row-span-4"}  md:ml-4 mt-2 mb-3 md:mt-6 md:mb-6 text-white text-3xl items-center space-x-1 md:space-y-4`}
        >
            <img src={incomplete? decoracionMin : Decoracion} className="hidden lg:block"/>
            {
                navItems.map(element => 
                    <Link to={element.link}
                        ...
                        style={element.style}
                    >
                        <img src={element.img} className={`w-8 h-8 lg:w-10 lg:h-10 ${!incomplete && "md:mr-1 md:ml-1 lg:mr-4 lg:mr-4"}`} />
                        { !incomplete && <label className="hidden md:block md:text-base lg:text-xl">{element.label}</label>} 
                    </Link>
                )
            }
            <img src={incomplete? decoracionMin : Decoracion} className="hidden lg:block"/>
