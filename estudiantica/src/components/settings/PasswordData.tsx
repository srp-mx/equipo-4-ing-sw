...
const PasswordInput = ({ value, onChange, text }: { value: string; onChange: (e: React.ChangeEvent<HTMLInputElement>) => void ; text : string}) => {
...
export default function PasswordData(){
    ...
    return (
        <div className="flex justify-center items-center ">
            ...
            <h1 className="text-3xl text-white mb-8">Cambiar contraseña</h1>
                {/* Form Section */}
                <div className="space-y-4">
                <PasswordInput value={currentPassword} onChange={(e) => setCurrentPassword(e.target.value)} text={"Contraseña actual"} />
                <PasswordInput value={newPassword} onChange={(e) => setNewPassword(e.target.value)} text={"Nueva contraseña"}/>
                <PasswordInput value={confirmPassword} onChange={(e) => setconfirmPassword(e.target.value)} text={"Confirmar contraseña"}/>
                </div>
